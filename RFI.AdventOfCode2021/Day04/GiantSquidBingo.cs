namespace RFI.AdventOfCode2021.Day04
{
    ...
    /// 
    /// The submarine has a bingo subsystem to help passengers (currently, you and the giant squid) pass the time.It automatically generates a random order in which to draw 
    ...
    /// You aren't sure how many bingo boards a giant squid could play at once, so rather than waste time counting its arms, the safe thing to do is to figure out which 
    /// board will win last and choose that one. That way, no matter which boards it picks, it will win for sure.
    /// 
    ...
    public class GiantSquidBingo
    {
        ...
        {
            (var drawnNumbers, var bingoBoards) = ParseConfiguration(gameConfiguration);
            foreach (var drawnNumber in drawnNumbers)
            {
                ...
                {
                    return boardsWithBingo.Max(b => b.GetScore());
                }
            }
        ...
        public int PlayWhenLastBoardWins(IEnumerable<string> gameConfiguration)
        {
            (var drawnNumbers, var bingoBoards) = ParseConfiguration(gameConfiguration);
            foreach (var drawnNumber in drawnNumbers)
            {
        ...
        private (IEnumerable<int> drawnNumbers, List<BingoBoard> bingoBoards) ParseConfiguration(IEnumerable<string> gameConfiguration)
        {
            var configuration = gameConfiguration.ToList();
            return (configuration[0].Split(',').Select(n => int.Parse(n)), CreateBingoBoards(configuration));
        }

        private List<BingoBoard> CreateBingoBoards(List<string> configuration)
        {
            var bingoBoards = new List<BingoBoard>();

            var boardNumbers = new List<int[]>();
            for (int i = 2; i < configuration.Count; i++)
            {
