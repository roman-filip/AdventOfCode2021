namespace RFI.AdventOfCode2021.Day09
{
    /// <summary>
    /// --- Day 9: Smoke Basin ---
    /// 
    ...
    /// 
    /// The risk level of a low point is 1 plus its height.In the above example, the risk levels of the low points are 2, 1, 6, and 6. The sum of the risk levels 
    ...
    public class SmokeBasin
    {
        ...
        public int ComputeRiskLevel(string[] heightmap)
        {
            ...
            for (_rowIndex = 0; _rowIndex < _heightmap.Length; _rowIndex++)
            {
                for (_colIndex = 0; _colIndex < _heightmap[_rowIndex].Length; _colIndex++)
                {
                    if (IsLowPoint())
                    {
                        lowPoints.Add(int.Parse(_heightmap[_rowIndex][_colIndex].ToString()));
                    }
                }
            }
        ...
        private bool IsLowPoint()
        {
            ...
            return c < GetNumber(_rowIndex, _colIndex - 1) && c < GetNumber(_rowIndex, _colIndex + 1)
                && c < GetNumber(_rowIndex - 1, _colIndex) && c < GetNumber(_rowIndex + 1, _colIndex);
        }

        private char GetNumber(int row, int col)
